var Springboard = function($config)
{
    ...
    {
        $this.Clock.Display.Time.html(moment().format(($this.Config.clock && $this.Config.clock.format) ? $this.Config.clock.format : 'H:mm<\e\m>:ss</\e\m>'));
    };
    ...
    {
        $this.Clock.Display.Weather = $('<div class="weather">').appendTo($this.Clock.Display);
        ...
        $this.Clock.Display.Weather.Update = function()
        {
            $.ajax(
            {
                data:
                ...
                success: function(data)
                {
                    if(data && data.list)
                    {
                        ...
                        {
                            icons.push('<span class="wi ' + weather_icon(data.list[i].weather[0].icon) + '"></span>');
                        }

                        $this.Clock.Display.Weather.html(icons.join('<span class="wi wi-right"></span>'));
                    }
                }
            });
        };
        ...
        setInterval(
            $this.Clock.Display.Weather.Update, 30 * 60 * 1000);
            $this.Clock.Display.Weather.Update();
    }
